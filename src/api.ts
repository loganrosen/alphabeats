import { norm, expandAddress } from './utils.js';
...
export async function searchRestaurants(params: SearchParams): Promise<ApiRow[]> {
  ...
  if (params.boro.length === 1) {
    ...
    conditions.push(`(${params.boro.map(b => `boro='${b.replace(/'/g, "''")}'`).join(' OR ')})`);
  }
  if (params.zip)     conditions.push(`zipcode='${params.zip}'`);
  if (params.cuisine) {
    const terms = params.cuisine.split(',').map(s => norm(s.trim())).filter(Boolean);
    if (terms.length === 1) {
      conditions.push(`upper(cuisine_description) like '%${terms[0].replace(/'/g, "''")}%'`);
    } else if (terms.length > 1) {
      const parts = terms.map(t => `upper(cuisine_description) like '%${t.replace(/'/g, "''")}%'`);
  ...
  if (params.address) {
    ...
    for (const token of expandAddress(params.address)) {
      conditions.push(`upper(building)||' '||upper(street) like '%${token.replace(/'/g, "''")}%'`);
    }
  }
...
export function groupRows(rows: ApiRow[]): Restaurant[] {
  ...
  for (const r of rows) {
    ...
    const year = r.inspection_date ? new Date(r.inspection_date).getFullYear() : 0;

...
export default function GradeTimeline({ inspections, onSelect }: Props) {
  ...
  // Y-axis: score ticks
  const yTicks = [0, 13, 27, scoreMax].filter(
    (v, i, a) => a.indexOf(v) === i && v <= scoreMax,
  );
  ...
  function tooltipTop(i: Inspection): number {
    ...
    const svgHeightPx =
      containerRef.current?.querySelector("svg")?.getBoundingClientRect()
        .height ?? H;
  ...
  return (
    <div className="mb-8">
      <h2 className="font-mono text-xs tracking-widest text-zinc-400 dark:text-zinc-500 uppercase mb-3 flex items-center gap-2">
        ...
        <GradeInfo />
        <span className="normal-case tracking-normal text-zinc-400 dark:text-zinc-600 font-normal">
          · lower violation points = better
        </span>
      </h2>
      <div
        ref={containerRef}
        className="rounded border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-visible relative"
      >
        <svg
          viewBox={`0 0 ${W} ${H}`}
        ...
        >
          ...
          {yTicks.map((score) => {
            const y = yOf(score);
            const color =
              score === 13 ? "#15803d" : score === 27 ? "#d97706" : undefined;
            return (
              <g key={score}>
                <line
                  x1={PAD_LEFT - 4}
                  ...
                  strokeWidth={score === 0 ? 0 : 0.5}
                  strokeDasharray={score === 0 ? undefined : "3 3"}
                  opacity={color ? 0.4 : 0.15}
                  className="text-zinc-500"
                />
                <text
                  x={PAD_LEFT - 7}
          ...
          {pts.map((insp, i) => {
            const cx = xOf(insp);
            const cy =
              insp.score != null ? yOf(insp.score) : PAD_TOP + chartH / 2;
        ...
        {hoveredInsp &&
          (() => {
            const critCount = hoveredInsp.violations.filter(
              (v) => v.critical,
            ).length;
            return (
              <div
                ...
                style={{
                  left: tooltipLeft(hoveredInsp),
                  top: tooltipTop(hoveredInsp),
                  transform: tooltipTransform(hoveredInsp),
                }}
              >
                <div className="bg-zinc-900 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-900 rounded px-2.5 py-1.5 font-mono text-xs whitespace-nowrap shadow-lg">
                  <div className="font-semibold">
                    {fmtDate(hoveredInsp.date)}
                  </div>
                  {hoveredInsp.grade && (
                    <div style={{ color: GRADE_COLOR[hoveredInsp.grade] }}>
                      Grade{" "}
                      {GRADE_TOOLTIP[hoveredInsp.grade] ??
                        GRADE_DISPLAY[hoveredInsp.grade] ??
                        hoveredInsp.grade}
                      {hoveredInsp.score != null
                        ? ` · ${hoveredInsp.score} pts`
                        : ""}
                    </div>
                  )}
                  {critCount > 0 && (
                    <div className="text-red-400 dark:text-red-600">
                      {critCount} critical violation{critCount !== 1 ? "s" : ""}
                    </div>
                  )}
                  {hoveredInsp.closed && (
                  ...
                  {hoveredInsp.reinspection && (
                    <div className="opacity-60">Re-inspection</div>
                  )}
                  {hoveredInsp.type && !hoveredInsp.reinspection && (

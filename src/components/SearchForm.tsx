...
import type { SearchParams, CommunityBoard } from '../api.js';
import CuisineCombobox from './CuisineCombobox.js';

const BOROUGHS = ['Manhattan', 'Brooklyn', 'Queens', 'Bronx', 'Staten Island'] as const;

const inputCls = "w-full bg-white border border-zinc-300 text-zinc-900 placeholder-zinc-400 rounded-md px-3 py-2.5 text-base focus:outline-none focus:border-yellow-500 transition-colors dark:bg-zinc-950 dark:border-zinc-600 dark:text-zinc-100 dark:placeholder-zinc-500 dark:focus:border-yellow-400";
const labelCls = "font-mono text-xs text-zinc-500 tracking-widest uppercase dark:text-zinc-300";
...
function CommunityBoardCombobox({ value, onChange, communityBoards, boroFilter }: {
  value: string; onChange: (code: string) => void; communityBoards: CommunityBoard[]; boroFilter: string[];
}) {
  const filtered = communityBoards.filter(cb =>
    boroFilter.length === 0 || boroFilter.some(b => cb.borough.toLowerCase() === b.toLowerCase())
  );
  const selected = communityBoards.find(cb => cb.code === value) ?? null;
  const [query, setQuery] = useState('');

  const matches = query
    ? filtered.filter(cb => cb.label.toLowerCase().includes(query.toLowerCase()))
    : filtered;

  const displayValue = (cb: CommunityBoard | null) => cb ? shortLabel(cb.label) : '';
  ...
  function highlight(text: string, q: string) {
    ...
    return <>{text.slice(0, idx)}<mark className="bg-yellow-200 dark:bg-yellow-700 text-inherit rounded-sm">{text.slice(idx, idx + q.length)}</mark>{text.slice(idx + q.length)}</>;
  }
  ...
  return (
    <Combobox value={selected} onChange={cb => { onChange(cb?.code ?? ''); setQuery(''); }} onClose={() => setQuery('')}>
      <ComboboxInput
        ...
        >
          {matches.map(cb => (
            ...
            >
              {({ selected: sel }) => (
                <>
                  <span className="leading-snug">{highlight(cb.label, query)}</span>
                  <span className="text-zinc-400 dark:text-zinc-500 text-xs shrink-0">{cbShort(cb.code)}{sel ? ' ✓' : ''}</span>
                </>
              )}
            </ComboboxOption>
          ))}
...
export default function SearchForm({ values, onChange, onSearch, onClear, loading, cuisines, communityBoards }: Props) {
  const set = (key: keyof SearchParams) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
    onChange(v => ({ ...v, [key]: e.target.value }));
  const onKey = (e: React.KeyboardEvent) => { if (e.key === 'Enter') onSearch(); };
    ...
    <div className="bg-zinc-100 border-b border-zinc-200 px-8 py-4 dark:bg-zinc-900 dark:border-zinc-800">
      <div className="grid grid-cols-[2fr_1.2fr_1.5fr_0.9fr_auto] gap-3 max-w-6xl items-end max-[800px]:grid-cols-1">
        <div className="flex flex-col gap-1.5">
          ...
          <input className={inputCls} value={values.name} onChange={set('name')} onKeyDown={onKey} placeholder="e.g. Drunken Munkey" />
        </div>
        <div className="flex flex-col gap-1.5">
          <label className={labelCls}>Borough</label>
          <div className="flex gap-1.5 flex-wrap py-0.5">
            {BOROUGHS.map(b => {
              const short = b === 'Staten Island' ? 'SI' : b === 'Manhattan' ? 'MN' : b === 'Brooklyn' ? 'BK' : b === 'Queens' ? 'QN' : 'BX';
              ...
              return (
                <button
                  ...
                  onClick={() => onChange(prev => ({
                    ...prev,
                    boro: active ? prev.boro.filter(x => x !== b) : [...prev.boro, b],
                    cb: '',
                  }))}
                  title={b}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input className={inputCls} value={values.address} onChange={set('address')} onKeyDown={onKey} placeholder="e.g. 338 E 92nd St" />
        </div>
        <div className="flex flex-col gap-1.5">
          <label className={labelCls}>Zip Code</label>
          <input className={inputCls} value={values.zip} onChange={set('zip')} onKeyDown={onKey} placeholder="e.g. 10128" maxLength={5} />
        </div>
        <button
          onClick={onSearch}
          disabled={loading}
      ...
      <div className="grid grid-cols-[1.5fr_1.2fr_0.8fr_auto] gap-3 max-w-6xl mt-3 items-end max-[800px]:grid-cols-1">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex gap-1.5 flex-wrap py-0.5">
            {([['A','A'],['B','B'],['C','C'],['N','N/A'],['Z','Pend']] as [string,string][]).map(([code, label]) => {
              ...
              return (
                <button
                  ...
                  onClick={() => onChange(prev => ({
                    ...prev,
                    grade: active ? prev.grade.filter(g => g !== code) : [...prev.grade, code]
                  }))}
                  className={`font-mono text-xs px-2.5 py-1.5 rounded-md border transition-colors cursor-pointer
                    ${active

...
export default function CuisineCombobox({ value, onChange, cuisines, className }: Props) {
  ...
  return (
    ...
    >
      {selected.map(c => (
        <span key={c} className="flex items-center gap-1 bg-yellow-100 text-yellow-900 dark:bg-yellow-900/30 dark:text-yellow-300 font-mono text-xs px-2 py-0.5 rounded-full shrink-0">
          {c}
          <button
            type="button"

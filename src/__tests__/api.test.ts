...
import { searchRestaurants, fetchByCamis } from '../api.js';
...
describe('searchRestaurants — WHERE clause construction', () => {
  ...
  it('filters by cuisine', async () => {
    ...
    expect(capturedWhere(mock)).toContain("upper(cuisine_description) like '%JAPANESE%'");
  });
...
describe('fetchByCamis', () => {
  it('queries by camis and returns grouped restaurant', async () => {
    const rows = [{
      camis: '50000001', dba: 'TEST PLACE', boro: 'Manhattan',
      building: '1', street: 'MAIN ST', zipcode: '10001',

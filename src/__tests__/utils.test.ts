...
import { norm, expandAddress, fmtDate } from '../utils.js';

describe('norm', () => {
  it('uppercases', () => expect(norm('hello')).toBe('HELLO'));
  it('collapses whitespace', () => expect(norm('  foo   bar  ')).toBe('FOO BAR'));
...
describe('fmtDate', () => {
  ...
  it('returns em dash for undefined', () => expect(fmtDate(undefined)).toBe('—'));

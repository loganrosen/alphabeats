...
import { type ApiRow, groupRows } from "../api.js";

// Minimal factory for ApiRow-shaped objects
function row(overrides: Partial<ApiRow> = {}): ApiRow {
	return {
		camis: "50000001",
		dba: "TEST PLACE",
		boro: "Manhattan",
...
describe("groupRows", () => {
	...
	it("creates separate inspections for different dates", () => {
		const rows = [
			...
			row({
				inspection_date: "2023-06-01T00:00:00.000",
				grade: "B",
				score: "20",
				violation_code: "06D",
			}),
	...
	it("marks critical violations", () => {
		...
		const [rest] = groupRows([r]);
		expect(Object.values(rest.inspections)[0].violations[0].critical).toBe(
			true,
		);
	});
	...
	it("sorts results: A grade before B before C, then alpha", () => {
		...
		const results = groupRows(rows);
		expect(results.map((r) => r.dba)).toEqual([
			"ALPHA",
			"GAMMA",
			"BETA",
			"ZITI",
	...
	it("parses score as integer", () => {
		const [rest] = groupRows([row({ score: "27" })]);
	...
	it("handles rows with no violation code (inspection with no violations)", () => {
		const r = {
			...row(),
			violation_code: undefined,
			violation_description: undefined,
		};
		const [rest] = groupRows([r]);
